...
def creation_scatter_dataset_pred(test_dataset:pd.DataFrame, forecast_series:pd.Series):
    """This function creates the dataset for the scatter plot for the predictions.  For every column (except OilPeakRate) will have a positive and negative version.
    OilPeakRate is here a binary indicating if the prediction is good or bad.
    ...
    scatter_dataset['OilPeakRate'] =  (scatter_dataset['OilPeakRate']!=forecast_series.to_numpy()).astype(int)
...
def creation_histo_full_pred(test_dataset:pd.DataFrame,forecast_series:pd.Series):
    ...
    histo_full['OilPeakRate'] =  (histo_full['OilPeakRate']!=forecast_series.to_numpy()).astype(int)
    histo_full.columns = histo_full.columns.str.replace('OilPeakRate', 'PREDICTION')
...
<|{distrib_class}|chart|title=Distribution between OilPeakRate and Stayed|values=values|labels=labels|type=pie|layout={mm_pie_color_dict_2}|>
|f1_score>
|>
|Metrics>
